...
def cctv_frame(name, id_number, cam_id, location, ts, filename):
    ...
    # 地板與走道透視線
    d.polygon([(0, H), (W, H), (W // 2 + 160, 300), (W // 2 - 160, 300)], fill=(34, 40, 50))
    d.polygon([(W // 2, 300), (W // 2 - 160, 300), (0, H), (W // 2 - 90, H)], fill=(30, 36, 46))
    d.polygon([(W // 2, 300), (W // 2 + 160, 300), (W, H), (W // 2 + 90, H)], fill=(26, 32, 42))

    # 人物剪影（監視畫面中的人）
    px = W // 2
    d.ellipse([px - 34, 150, px + 34, 218], fill=(8, 10, 12))          # 頭
    d.polygon([(px - 46, 300), (px + 46, 300), (px + 40, 400), (px - 40, 400)], fill=(10, 12, 14))  # 身體
    ...
    d.text((14 + len(name) * 12 + 10, H - 42), id_number, font=font(14), fill=(170, 220, 170))
...
cctv_frame("張三", "A118153566", "CAM-01 ｜ 大門口", "一樓大廳入口", "20260919_15:30:52", "A118153566.jpg")
cctv_frame("陳小美", "B223456789", "CAM-02 ｜ 電梯口", "東側電梯前", "20260919_15:28:10", "B223456789.jpg")
cctv_frame("李大明", "C123456789", "CAM-03 ｜ 停車場", "地下停車場 B2", "20260919_15:25:44", "C123456789.jpg")
cctv_frame("王阿豪", "D123456789", "CAM-04 ｜ 走道", "三樓走道", "20260919_15:22:07", "D123456789.jpg")